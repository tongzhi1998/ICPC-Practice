...
int main()
{
	...
	ifstream ifile("f_libraries_of_the_world.txt");
	ifile>>B>>L>>D;
	values.resize(B);
	for(int i=0;i<B;++i)
	{
	...
	for(int i=0;i<libs.size();++i)
	{
			for(int j=i;j<libs.size();++j)
			{
				...
				for(int k=libs[j].currPos;k<libs[j].books.size();++k)
				{
					...
					{
						if(currD+ceil((k-libs[j].currPos)/libs[j].speed) >=D)
							break;
			...
			for(int t=0;t<tLimit;++t)
			{
				for(int s = 0; s<libs[j].speed;++s)
				{
					if(visited.find(libs[j].books[libs[j].currPos])==visited.end())
					{
						...
						libs[j].p.insert(libs[j].books[libs[j].currPos]);
						libs[j].currPos++;
	...
	{ 	
		...
		ofile << libs[i].index << " " << libs[i].books.size() << endl;
		...
		{
			if(libs[i].p.find(libs[i].books[j])==libs[i].p.end())
				ofile << libs[i].books[j] << " ";
		}
		ofile << endl;
	}
